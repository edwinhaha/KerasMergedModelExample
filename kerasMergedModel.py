...
labelsFlat = data['label'] # [[0]].values
...
leftBranch.add(Conv2D(classes, (3,1), activation='relu'))
...
rightBranch.add(Conv2D(classes, (1,3), activation='relu'))
...
centralBranch.add(Conv2D(classes, (5,5), activation='relu'))
...
model.summary()
model.fit([images,images,images], labelsCategorical, epochs=5, batch_size=100, verbose=1)

yPred = model.predict_classes([testX,testX,testX])

np.savetxt('dr.csv', np.c_[range(1,len(yPred)+1),yPred], delimiter=',', header='ImageId,Label', comments ='', fmt='%d')
